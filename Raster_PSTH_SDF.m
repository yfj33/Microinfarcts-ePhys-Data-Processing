...
%load('sptimes.mat')
clc;clear;close all
load("cell_type_2023.mat");
sptimes=cell_type.spikes_MS.times;
%sptimes=sptimes(1,1:10);
%% Raster plot
...
for iTrial = 1:length(sptimes)
    ...
    spks_s=spks(spks<20);
    xspikes         = repmat(spks_s,3,1);         % Replicate array
    yspikes      	= nan(size(xspikes));       % NaN array
    
    if ~isempty(yspikes)
        yspikes(1,:) = iTrial-0.2;                % Y-offset for raster plot
        yspikes(2,:) = iTrial;
    end
