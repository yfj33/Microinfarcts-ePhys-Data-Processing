% the script aims to plot the output of mountainsort after initial
...
% 2022/08/16 for 32 channels
clc;clear;close all
cmap=load('/media/luanlab/DATA/SpikeSorting/SpikeSorting_Script/Channel_Map/pcb_finger_channel_map_8x4_32ch.mat');
files=dir;
files(1:2)=[];
...
save_path=fullfile(pwd,'clusters figures');
...
binrange_isi=[0:0.5:100]; %bin size should be 0.5ms to visualize 2ms vilation
binrange_acg=[-100:0.5:100]; 
...
for i=1:ClustN
    ...
    for j=1:ch_alive ;%total available channels
       if ch_native(j)==PC_map; %plot primary channel number as red 
    [r c]=find(ch_native(j)==CMN);
    ...
    %subplot(m+3,n,seq);
    plot(InT,mw(j,:,i),'Color','r','LineWidth',1.5);%plot around 3ms interval time average waveform, filled in 32*4 plot
    pbaspect([3 2 1]);
     xlim([0 3.4]);
    ylim([-200 200]);
    ylabel('V (μV)');
    ...
    %pbaspect([1 1 1]);
     ...
     nexttile(seq);
     %subplot(m+3,n,seq);
     plot(InT,mw(j,:,i),'Color','b','LineWidth',1.5); 
     pbaspect([3 2 1]);
    xlim([0 3.4]);
    ylim([-200 200]);
    ylabel('V (μV)');
